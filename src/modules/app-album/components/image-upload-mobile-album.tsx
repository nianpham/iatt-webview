...
const ImageUploadMobileAlbum = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (totalImages > MAX_IMAGES) {
      toast({
        title: "Lỗi",
        description: `Tổng số ảnh không thể quá ${MAX_IMAGES}. Hiện tại đã có ${newImages.length} ảnh, bạn đang cố thêm ${fileArray.length} ảnh.`,
        variant: "destructive",
      });
      return;
    }
    ...
    >
      ...
      <div className="relative group w-full h-full">
        ...
        {newImages.length === 0 && (
          <div
            onClick={handleClick}
            className="flex h-[300px] w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-gray-300 bg-white px-5 py-3 mt-5 text-sm font-medium text-gray-900 hover:bg-gray-50 hover:text-primary-700 cursor-pointer"
          >
